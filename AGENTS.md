...
- `/flights` (September 20) shows KL ⇄ London fares from `flights.json` (scraper `flight_prices.py`, Google Flights, copied by Hermes): direction and trip toggles, headline tiles, a fare-by-date line chart with crosshair tooltip, a price-calendar heatmap, a detail panel, and month/all-dates tables. Classes built from variables (`fx-seq-*`, `fx-badge-*`) live outside `@layer` in `globals.css` so Tailwind does not purge them. Do not commit `flights.json` or other scraper data files from a worktree: the live folder's copies change daily and block fast-forward merges.
