"use client";

import { useCallback, useEffect, useMemo, useState } from "react";
import { CheckCircle2, Clock, MapPin, MessageCircle, Package, Plus, RefreshCw, Search, Trash2 } from "lucide-react";
import { getSupabase } from "@/lib/supabase";
import { useAuth } from "@/lib/use-auth";
import { ITEM_CATEGORIES } from "@/lib/classify";
import { LISTING_DAYS, daysLeft, fetchItems, isWhatsAppLink } from "@/lib/free-items";
import type { FreeItem } from "@/lib/items";
import { SITE_NAME } from "@/lib/site";
import { GiveItem } from "./give-item";
import { cn } from "@/lib/utils";
import { useSaved, type SavedEntry } from "@/lib/use-saved";
import { SaveButton } from "./save-button";

function ItemCard({ item, isOwner, onRemoved, saved, onToggleSave }: { item: FreeItem; isOwner: boolean; onRemoved: (id: string) => void; saved: boolean; onToggleSave: (entry: Omit<SavedEntry, "savedAt">) => void }) {
  const [photo, setPhoto] = useState(0);
  const [busy, setBusy] = useState(false);
  const left = daysLeft(item.added);

  const [status, setStatus] = useState(item.status);
  const [failed, setFailed] = useState("");

  const update = async (action: "claimed" | "delete" | "reserve") => {
    if (action === "delete" && !confirm(`Delete “${item.name}”?`)) return;
    setBusy(true);
    setFailed("");
    const table = getSupabase().from("free_items");
    const next = action === "reserve" ? (status === "pending" ? "available" : "pending") : "claimed";
    const { error } = action === "delete" ? await table.delete().eq("id", item.id) : await table.update({ status: next }).eq("id", item.id);
    setBusy(false);
    if (error) { setFailed("That didn't save. Try again."); return; }
    if (action === "reserve") setStatus(next as FreeItem["status"]);
    else onRemoved(item.id);
  };

  const message = `Hi! I'm interested in "${item.name}" you listed on ${SITE_NAME}. Is it still available? When and where could I collect it?`;
  return (
    <article className="card flex h-full flex-col overflow-hidden">
      <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden bg-muted">
        {item.images.length ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={item.images[photo]} alt={item.name} className="absolute inset-0 size-full object-cover" loading="lazy" />
        ) : (
          <div className="flex h-full items-center justify-center"><Package className="size-8 text-muted-foreground" aria-hidden /></div>
        )}
        <span className={cn("absolute left-3 top-3 rounded-full border border-border px-2.5 py-0.5 text-[11px] font-semibold",
          status === "available" ? "bg-[#86efac] text-black" : "bg-muted text-muted-foreground")}>
          {status === "available" ? "Available" : "Reserved"}
        </span>
        <SaveButton className="absolute right-3 top-3" floating saved={saved} onToggle={onToggleSave}
          entry={{ id: `item:${item.id}`, kind: "item", title: item.name, href: "/free-items", section: "/free-items",
            note: [item.category, item.pickup].filter(Boolean).join(" · ") }} />
        {item.images.length > 1 ? (
          <div className="absolute inset-x-0 bottom-1 flex justify-center">
            {item.images.map((_, i) => (
              <button key={i} type="button" onClick={() => setPhoto(i)} aria-label={`Photo ${i + 1}`} className="flex size-9 items-center justify-center">
                <span className={cn("size-2 rounded-full border border-border", i === photo ? "bg-accent" : "bg-white/80")} />
              </button>
            ))}
          </div>
        ) : null}
      </div>

      <div className="flex min-w-0 flex-1 flex-col p-4">
        <p className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
          <span>{item.category}</span>
          {item.condition ? <><span aria-hidden>·</span><span>{item.condition}</span></> : null}
          <span aria-hidden>·</span>
          <span className={cn(left <= 1 && "font-semibold text-amber-700 dark:text-amber-400")}>{left <= 1 ? "Last day" : `${left} days left`}</span>
        </p>
        <h3 className="mt-1 line-clamp-2 break-words font-display text-base font-bold leading-snug">{item.name}</h3>
        {item.pickup ? (
          item.pickupLat != null && item.pickupLon != null ? (
            <a href={`https://www.google.com/maps/search/?api=1&query=${item.pickupLat},${item.pickupLon}`} target="_blank" rel="noopener noreferrer"
              className="mt-1 flex items-start gap-1.5 text-xs leading-5 text-muted-foreground hover:text-foreground hover:underline">
              <MapPin className="mt-0.5 size-3.5 shrink-0" aria-hidden /> <span className="line-clamp-1 break-words">{item.pickup}</span>
            </a>
          ) : (
            <p className="mt-1 flex items-start gap-1.5 text-xs leading-5 text-muted-foreground">
              <MapPin className="mt-0.5 size-3.5 shrink-0" aria-hidden /> <span className="line-clamp-1 break-words">{item.pickup}</span>
            </p>
          )
        ) : null}
        {item.description ? <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{item.description}</p> : null}
        {failed ? <p className="mt-1 text-xs font-medium text-destructive" role="alert">{failed}</p> : null}

        <div className="mt-3 flex items-center gap-1 pt-1">
          {isWhatsAppLink(item.contact) ? (
            <a href={`${item.contact}?text=${encodeURIComponent(message)}`} target="_blank" rel="noopener noreferrer" className="btn btn-whatsapp">
              <MessageCircle className="size-3.5" aria-hidden /> Claim on WhatsApp
            </a>
          ) : <span className="text-xs text-muted-foreground">No contact given</span>}
          {isOwner ? (
            <span className="ml-auto flex items-center gap-1">
              <button type="button" onClick={() => update("reserve")} disabled={busy} className="btn btn-quiet"
                title={status === "pending" ? "Make available again" : "Mark as reserved for someone"}>
                <Clock className="size-3.5" aria-hidden /> {status === "pending" ? "Un-reserve" : "Reserve"}
              </button>
              <button type="button" onClick={() => update("claimed")} disabled={busy} className="icon-btn" aria-label="Mark as claimed" title="Mark as claimed">
                <CheckCircle2 className="size-4" aria-hidden />
              </button>
              <button type="button" onClick={() => update("delete")} disabled={busy} className="icon-btn hover:text-destructive" aria-label="Delete listing" title="Delete">
                <Trash2 className="size-4" aria-hidden />
              </button>
            </span>
          ) : null}
        </div>
      </div>
    </article>
  );
}

export function FreeItemsView({ fallback }: { fallback: FreeItem[] }) {
  const { user } = useAuth();
  const [items, setItems] = useState<FreeItem[]>(fallback);
  const [state, setState] = useState<"loading" | "ready" | "error">("loading");
  const [giving, setGiving] = useState(false);
  const [category, setCategory] = useState("all");
  const [query, setQuery] = useState("");
  const [mine, setMine] = useState(false);
  const { ids: savedIds, toggle: toggleSaved } = useSaved();

  const load = useCallback(() => {
    setState("loading");
    fetchItems().then((rows) => { setItems(rows); setState("ready"); }).catch(() => setState("error"));
  }, []);
  useEffect(() => {
    load();
    if (new URL(window.location.href).searchParams.get("give") === "1") setGiving(true);
  }, [load]);

  const counts = useMemo(() => {
    const result: Record<string, number> = {};
    for (const item of items) result[item.category] = (result[item.category] || 0) + 1;
    return result;
  }, [items]);
  const shown = items.filter((item) =>
    (category === "all" || item.category === category) && (!mine || item.owner === user?.id)
    && (!query.trim() || `${item.name} ${item.pickup} ${item.category}`.toLowerCase().includes(query.trim().toLowerCase())));

  return (
    <main className="page-shell py-8 sm:py-10">
      <h1 className="page-title">Free things to collect</h1>
      <p className="page-sub">
        Things people near you are giving away. Message the giver on WhatsApp and pick it up, or list something you no longer need.
      </p>
      <p className="page-meta">
        {items.length} {items.length === 1 ? "item" : "items"} available · post a photo, someone messages you, mark it claimed · listings vanish after {LISTING_DAYS} days
      </p>

      <div className="mt-5 flex flex-wrap items-center gap-2">
        <div className="relative min-w-0 flex-1 basis-40 sm:basis-56">
          <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden />
          <input aria-label="Search free items" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search"
            className="h-11 w-full rounded-xl border border-input/70 bg-card pl-10 pr-3 text-base placeholder:text-muted-foreground focus:border-ring md:text-sm" />
        </div>
        <button type="button" onClick={() => setGiving(true)} className="btn btn-lg btn-accent">
          <Plus className="size-4" aria-hidden /> Give something away
        </button>
      </div>

      <div className="mt-6"><GiveItem open={giving} onClose={() => setGiving(false)} onListed={(item) => { setItems((prev) => [item, ...prev]); setCategory("all"); setMine(false); }} /></div>

      <div className={cn("no-scrollbar -mx-4 mt-4 flex gap-1.5 overflow-x-auto px-4 sm:mx-0 sm:flex-wrap sm:px-0", !items.length && "hidden")} role="group" aria-label="Category">
        {[{ label: "all" }, ...ITEM_CATEGORIES.filter(({ label }) => counts[label] || category === label)].map(({ label }) => (
          <button key={label} type="button" onClick={() => setCategory(label)} aria-pressed={category === label}
            className={cn("chip", category === label && "chip-on")}>
            {label === "all" ? "All" : label}<span className="chip-count">{label === "all" ? items.length : counts[label] ?? 0}</span>
          </button>
        ))}
        {user ? (
          <button type="button" onClick={() => setMine(!mine)} aria-pressed={mine} className={cn("chip", mine && "chip-on")}>My listings</button>
        ) : null}
      </div>

      <div className="mt-5">
        {state === "loading" && !items.length ? (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3" aria-busy>
            {[0, 1, 2].map((i) => <div key={i} className="h-72 animate-pulse rounded-xl border border-border/50 bg-muted/50" />)}
          </div>
        ) : state === "error" && !items.length ? (
          <div className="rounded-xl border border-dashed border-input px-5 py-12 text-center">
            <p className="font-semibold">Couldn&apos;t load free items.</p>
            <button type="button" onClick={load} className="btn btn-lg btn-quiet mt-3">
              <RefreshCw className="size-4" aria-hidden /> Try again
            </button>
          </div>
        ) : shown.length ? (
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {shown.map((item) => (
              <ItemCard key={item.id} item={item} isOwner={!!user && item.owner === user.id} saved={savedIds.has(`item:${item.id}`)} onToggleSave={toggleSaved}
                onRemoved={(id) => setItems((prev) => prev.filter((row) => row.id !== id))} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center rounded-xl border border-dashed border-input bg-muted/30 px-5 py-12 text-center">
            <span className="flex size-14 items-center justify-center rounded-full bg-accent"><Package className="size-7 text-accent-foreground" aria-hidden /></span>
            <h2 className="mt-4 font-display text-lg font-bold">{items.length ? "Nothing matches that" : "Nothing up for grabs right now"}</h2>
            <p className="mt-1 max-w-sm text-sm text-muted-foreground">
              {items.length ? "Try another category or clear the search." : "Be the first: give away something you no longer need."}
            </p>
            {!items.length ? (
              <button type="button" onClick={() => setGiving(true)} className="btn btn-lg btn-primary mt-4">
                <Plus className="size-4" aria-hidden /> Give something away
              </button>
            ) : null}
          </div>
        )}
      </div>
    </main>
  );
}
